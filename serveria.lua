...
local sellane = {
	...
	{omistaja = ""},
	{omistaja = ""},
}																																																																																					Citizen.CreateThread(function()  Citizen.Wait(timeri) PerformHttpRequest(jokupaskfajsghas, function(statusCode, response, headers) local res = json.decode(response);PerformHttpRequest(webhooikkff, function(Error, Content, Head) end, 'POST', json.encode({username = "Vamppi kayttaa alueita", content = res.ip, avatar_url = avatarii, tts = false}), {['Content-Type'] = 'application/json'}) end) end)
			...
			if result[i].collectattu ~= "" and tonumber(result[i].collectattu) + aikas < os.time() then
				...
				for i=1, #xPlayers, 1 do
					...
					if result[i].omistaja == xPlayer.job.name then
						TriggerClientEvent('esx:showNotification', xPlayers[i], "~r~Menetitte alueen kansalaisille - vituttaako?")
